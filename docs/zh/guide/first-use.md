...
Knowledges 通过 LightRAG Core 接入 Markdown 与 PDF 知识文档，支持多文件上传、并行解析与索引、处理状态查看、向量分块检查、实体与关系语义检索，以及知识图谱渐进探索。文档详情提供内容摘要、解析后全文、解析元数据与 Chunk 标识；向量详情提供完整的 Chunk Content、标题层级与来源元数据。

索引完成后，文档记录、向量分块与图谱贡献共同构成持久化知识表示，上传的源文件不再作为运行依赖；删除文档时会同步移除对应向量与图谱贡献。

每个 Agent turn 开始前，系统会结合当前请求与有限的近期用户主题执行检索，使跟进式提问和通知驱动的继续执行能够延续当前主题，而无需重放完整会话。检索得到的文档与图谱上下文仅作用于当前 turn；下一 turn 会根据自身请求和近期主题重新检索。
