...
该架构以 FastAPI 作为控制平面，统一管理会话、项目和执行资源。核心运行时由智能体会话驱动，通过会话智能体编排图组织主控智能体与专家智能体，并由工具编排层连接 sandbox 执行、项目记录、异步任务和事件时间线。分布式 sandbox 资源为授权安全测试提供隔离执行环境，并在同一边界内提供浏览器访问、文件访问、受控 egress、sandbox 内 skills 和预装安全工具链；WorkProject 则将资产、漏洞发现和攻击路径沉淀为可追踪、可复盘的项目证据。底层通过 PostgreSQL 持久化会话状态、任务进度、漏洞发现、资产关系、攻击路径和回放事件，确保整体流程可追踪、可审查。
